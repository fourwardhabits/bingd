    ...
    when p_canary_user is null then exists (
      select 1 from profiles p
       where p.id = p_user
         ...
         and p.created_at <= now() - make_interval(hours => coalesce((select (c.value #>> '{}')::integer from app_config c where c.key = 'welcome.delay_hours'), 48))
