...
export async function run({
  ...
  /**
   * WHICH BACKEND THIS IS ABOUT TO TOUCH, PRINTED BEFORE ANYTHING ELSE.
   *
   * A project ref is the subdomain of the Supabase URL and is not a secret — it is in
   * every client build — while the service-role key beside it is, and is never printed.
   * `trending-refresh.yml` logs the same thing for the same reason: the mistake actually
   * available to somebody at a terminal is running the right command against the wrong
   * project, and one line naming it is what makes that visible before the send rather
   * than in the ledger afterwards.
   *
   * It matters most for the staging rehearsal, where the window is deliberately
   * shortened (README, "Rehearsing the cohort without waiting 48 hours"): the same shell
   * with the production URL still exported is how a shortened window reaches real
   * accounts, and this line is printed above the window it would shorten.
   */
  const projectRef = (() => {
    try {
  ...
  if (!projectRef) return stop(`SUPABASE_URL is not a URL.`, 2);
  log(`\n  project           ${projectRef}`);
