...
describe('every lane still claims bingd.app, which is a decision not an oversight', () => {
  /**
   * `bc225f9` gated the domain on the variant so a staging build would not assert the
   * marketing domain. Only the ICON half of that commit was restored (see
   * `docs/release/next-binary-checklist.md` §2, which says to decide the gating
   * separately rather than let it arrive with an icon). So the current, deliberate state
   * is the one `main` has always had: all four lanes declare the links.
   *
   * Asserted rather than left silent because the store lanes are the half that matters —
   * dropping beta's deep links would fail nothing at build time and would simply stop
   * `bingd.app/u/...` opening the app for every closed tester.
   */
  for (const lane of LANES) {
    it(lane + ' declares the universal links', () => {
      const exp = resolveConfig(lane);
