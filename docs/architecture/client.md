...
### v1 Open Graph cards are typographic — Required

**Decided 2026-08-13, under the approved gate change.** `og-render` composes ordinals, titles, and the wordmark. It embeds **no poster artwork**.

The two share paths look similar and are not. An on-device card is rendered on the user's phone from artwork already fetched for display, and the user shares it themselves — ordinary use of the images, which is what provider attribution terms contemplate. An Open Graph card is generated by Bingd's own server and served from Bingd's own infrastructure to any crawler that asks, indefinitely, with no user in the loop. PRD §19 says artwork is *"never rehosted on Bingd infrastructure,"* and a server-rendered image containing a poster is rehosting it. The two statements could not both be kept.

Typographic cards resolve it without needing a licensing answer, and they cost little: a link preview showing `#3 · The Zone of Interest` in DM Serif Display on Parchment is more distinctly Bingd than a poster grid would be, since the poster is the one element every competitor also has. Poster-bearing OG cards can be revisited once a commercial plan is in place and the question has a definite answer.

Two operational notes for `og-render`, both of which are the sort of thing that fails at store-submission time: **fonts must be embedded as files in the function bundle**, because the brand SVGs' remote Google Fonts `@import` fails silently server-side exactly as it does in an app bundle (PRD §5, C20); and rendered cards are cached by URL with the token as the key, so a crawler hit does not become a render on every scrape.
