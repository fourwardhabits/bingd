...
> choose. Cloudflare looks for `functions/` in the project's **root directory**, which
> this document says plainly is inferred rather than confirmed, so its placement is the
> one part of the Lists tranche to check on a preview URL before production — and
> **check it by unfurling a preview link, not by loading the page.** A misplaced Function
> is invisible to a browser, which renders the static page either way, and shows up only
> as a generic card in a message thread.
