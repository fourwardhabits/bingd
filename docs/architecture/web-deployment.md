...
> **Both of those sentences are now stale, and this whole section is a snapshot of
> 2026-08-14 rather than a description of today.** Two corrections, dated 2026-09-20:
>
> - **The AASA typo is fixed and deployed.** `/lists/*` was verified live in both the
>   Apple App Site Association file and `assetlinks.json` on 2026-09-19. Neither the
>   `/list/*` paragraph above nor the "Only `/lists/*` is affected by the typo" note
>   further down still applies; both are kept because the table around them describes a
>   deployment that genuinely existed, and the CDN-caching advice beside them is still
>   good.
> - **`/lists/<id>` no longer falls back to the generic page.** Since Lists v1 it renders
>   the list: title, attribution, description, the first 100 rows, and "See all N in the
>   app". It is the **one route on this site that draws a person's own words**, so its
>   rule is absolute — every user string is set with `textContent`, and no `innerHTML`
>   path touches a title, a description or a name. What it may read is bounded by
>   `_list_readable(id, null)`, the anonymous branch of the same predicate the app obeys,
>   so a private, hidden, deleted or suspended list is the same zero rows from out here
>   and keeps the generic page. `/lists` itself and `/lists/by/<uuid>` are deliberately
>   **not** matched by `listIdFromPath` and keep the install card: the first is the app's
>   management screen and the second its See-all, and neither is an object anybody
>   shares.
>
> **There is also a Pages Function now**, at `functions/lists/[id].js`, which rewrites
> four `<meta>` tags so a shared list unfurls as itself. It holds the anon key only, it
> falls back to the static card on every failure, and it names the owner **only when the
> owner's profile is public** — a third-party unfurl cache is a place the owner did not
> choose.

### The Pages root directory, settled by measurement (2026-09-20)

**`functions/` at the repository root is correct. This is no longer inferred.**

It was tested rather than reasoned about, because a misplaced Function is invisible: the
static page renders either way, so "the page loads" proves nothing, and the Function's own
fallback is *by design* indistinguishable from it never having run. Two identical probes
were deployed on a branch, one at `functions/_probe.js` and one at `web/functions/_probe.js`,
each naming its own location:

```
GET https://integration-lists-watch-hist.bingd.pages.dev/_probe
  -> 200  "root: repository root (functions/)"  x-bingd-probe: repo-root
```

The `web/` probe never answered. Both were deleted in the following commit; the result is
recorded here so nobody repeats the experiment.

**One inference that points the wrong way, worth recording.** A `POST` to `/lists/<uuid>`
returns a bare `405`, identically to `POST /title/<uuid>` — which reads as "no Function
here". It is not: `onRequest` calls `next()`, the static asset server answers a POST with
405, and the Function passes that through untouched. **Method behaviour cannot distinguish
a deployed Function from a missing one on this site.**

### What a preview URL still cannot demonstrate, and the one step that fixes it

**The Supabase variables are scoped to the Production environment only.** A preview
deployment's config block reads `"supabaseUrl": null` where production reads
`https://abheeqyjzekiowkztfxv.supabase.co`. So on a preview URL:

- the page renders and skips its fetch, showing the generic card;
- the Function runs, finds no `env.EXPO_PUBLIC_SUPABASE_URL`, and returns the response
  unchanged — its correct fallback, and therefore also invisible.

Neither is a defect. **Adding `EXPO_PUBLIC_SUPABASE_URL` and `EXPO_PUBLIC_SUPABASE_ANON_KEY`
to the Pages project's Preview environment is the one outstanding operational step** before
a preview URL can show a rendered list or a real unfurl. Cloudflare dashboard → Workers &
Pages → `bingd` → Settings → Environment variables → Preview. Point them at **staging**
(`fjxhcbowoxuzulwirzyr`), not production: a preview site reading production would make
`record_list_open` count real opens against real lists.

Until then the behaviour is verified one level down: every RPC the page and the Function
make has been exercised against real staging with the real anon key, over fixtures covering
public, link-only, link-only-with-a-private-owner, private and nonexistent lists.
