...
**Written 2026-08-29 against `54e32fd`; reconciled 2026-08-30 against `95fd4d7`, then `2f27577`, and again the same day against `37e8584`.** Only the things that are genuinely unresolved and
...
## 9. Added 2026-08-30 (`37e8584`) — two operational items, both sharper than they were

Neither blocks the RC. Both are things nothing in this repository will do on its own, and
**both were re-checked against reality for this pass rather than carried forward.**

- [ ] **Set the trending refresh secrets. The job is not missing — it is failing every
      night.** The previous version of this item said "`trending-refresh.yml` exists; the
      decision is whether to put it on a schedule." It **is** on a schedule, it has been
      running nightly, and it has **failed every single time** — five consecutive nights
      checked, each exiting in about thirty seconds with `SUPABASE_URL:` and
      `SUPABASE_SERVICE_ROLE_KEY:` both empty. The repository secrets were never set.

      This is a founder action because a service-role key is a credential and nothing in
      the repository holds one:

      | Secret | Value |
      |---|---|
      | `SUPABASE_URL_NONPROD` | `https://abheeqyjzekiowkztfxv.supabase.co` |
      | `SUPABASE_SERVICE_ROLE_KEY_NONPROD` | the nonprod service-role key, from the dashboard |

      Then **prove it, which is a separate step**: `gh workflow run trending-refresh.yml
      -f target=nonprod`, confirm the run reads **success**, and then confirm the *data* —
      `npm run test:remote` passes and all four `provider_list_cache` rows carry today's
      `fetched_at`. A green run that wrote nothing is the same dark shelf as a red one.

      **Until all three have passed, do not touch `trendingFallback`'s missing expiry
      check.** It reads the cache without checking `expires_at` while its sibling checks,
      which looks like a bug and is the only reason the cold-start half of the For You wall
      still draws anything: the lists expired on 2026-08-25. Respecting the expiry today
      empties the wall rather than refreshing it. The Feed's own Trending shelf *does*
      respect the cutoff, which is why it has been dark since then — that is the symptom to
      look for, and it is expected until this item is done. Runbook §15.2. TREND-1.

- [ ] **Run the deployed-function parity check after every release — it is now written
      down as required.** On 2026-08-30 the running `tmdb-adapter` was thirteen days
      behind the repository and every season row carried a null episode count, silently,
      with CI green throughout. Neither the OTA, nor `ci.yml`, nor the release gate, nor
      `scripts/release.mjs` deploys or checks a function.

      **Green CI does not prove the deployed function matches the merge SHA**, and the
      check therefore runs *after* the release rather than before it: for every function
      the release's diff touched, `updated_at` must be later than the merge commit.
      Runbook **§15.0** has the three commands, and the version numbers are the evidence to
      record — "deployed" is what somebody writes when they meant to.

      This tranche is the worked example: `37e8584` changed `push-sender`, which was at
      **v3** from 2026-08-25 and is now at **v4**. `tmdb-adapter` was untouched and
      correctly stayed at **v7**.
