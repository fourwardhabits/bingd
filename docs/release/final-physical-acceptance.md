...
**Written 2026-08-29 against `54e32fd`; reconciled 2026-08-30 against `95fd4d7`, and again the
same day against `2f27577`.** The last founder run on a real phone before the
release candidate. Everything below is a behaviour that shipped in the tranches from PR #63
to PR #77 and that **only a physical device can confirm** — nothing here is covered by the
...
| **The right update** | iOS group `5bce0b22-6265-4f34-b6a5-ef8bb29aac04` (update `01a054f0-e89d-725e-b581-3864decd9e4c`) · Android group `276afa35-9d3e-4d10-ade1-429a73d5a70b` (update `01a054f0-e89d-7247-b639-11aa38eceb03`) — both published from `2f27577` and messaged *"Leaderboard counts every logged title, Anime first in search…"* |
| **Runtime** | iOS `d3b308f74a08…` · Android `41a907174ba3…`. `2f27577` fingerprints to **byte-identical** values against `95fd4d7`, measured in one tree with one env by checking each SHA out over the other — and none of the 24 changed files is a fingerprint source or sits under one. Native delta **NONE**, **no new build needed for this run** |
| **Backend change** | This tranche has one: migration `20260903000100` is applied to nonprod (parity verified, remote smoke 105/105), and the **tmdb-adapter edge function was redeployed** for the first time since 2026-08-17. Both are already done — nothing to do on the phone |
