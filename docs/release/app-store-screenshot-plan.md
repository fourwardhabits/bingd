...
**Written 2026-08-29 against `54e32fd`; reconciled 2026-08-30 against `95fd4d7`, then `2f27577`, and again the same day against `37e8584`.** The shot list, the exact navigation for each
...
| | |
|---|---|
| **Screen** | Collection tab, Movies, Watched, sorted by **Rating** |
| **Navigation** | **Collection** tab → **Movies** on the medium selector → **Watched** segment → confirm the sort chip reads **Rating** with a **down** arrow (highest first). Since `37e8584` that is what the default order is called; a capture showing *Your score: high* or a two-headed sort glyph is from before the OTA and shows a control the app no longer has |
...
Two shots are affected, and only because the app now draws something it did not before.

| Shot | What changed | What to do |
|---|---|---|
| **Any TV or season screen** | Season rows carry an **episode count** ("9 episodes") for the first time. Every season row in the catalogue had a null count until 2026-08-30 — the metadata line simply omitted it. | Take the OTA first, then capture. A screenshot from before that date shows a line the shipped app draws differently. |
| **A title page for an anime film** | Genre pills now read **Anime** first, and never Animation. | Same: capture after the update. Choose a title where the pill order is visible — most surfaces slice to two genres, which is the reason the label was moved to the front. |
| **Leaderboard** | More accounts appear on the monthly board, because a logged title with no watch date now counts. | No change to the shot; the board is simply fuller, which is better for a screenshot than it was. |

Everything else in this plan stands. The build to capture on is unchanged — iOS build 5,
with the OTA from `2f27577` applied.

---

## Reconciled again 2026-08-30 — `37e8584`

One shot changes, and it changes because the control in it is different rather than because
the data behind it is.

| Shot | What changed | What to do |
|---|---|---|
| **Shot 3 — Collection, Movies, Watched** | The sort chip read *Your score: high* beside a two-headed glyph. It now reads **Rating** beside a **down arrow**, and the menu behind it has one row per axis rather than one per direction. *Recently watched* is gone from that menu entirely — it named a fact the list did not order by. | **Retake after applying the `37e8584` OTA.** A capture from before it shows a control the shipped app does not have. The composition and the navigation are otherwise unchanged. |
| **Any shot showing the profile See-all sheet** | The same change: *Rank · Highest first* is now **Rank** with a down arrow. | Retake if that sheet is visible in a submitted shot. It is not in the current list, so this is a guard rather than a task. |

**Nothing else moves.** No screen gained or lost an element, no copy changed, and the
award-lifecycle work in this tranche is invisible in a static capture — a revoked badge looks
exactly like a badge that was never earned, which is the point of it.

The build to capture on is unchanged — iOS build 5, with the OTA from `37e8584` applied.
