...
So the alpha catalogue is seeded from Wikidata: about 380 films, 190 series and 1,400 seasons, chosen by Wikipedia sitelink count, which is a serviceable proxy for "a tester will search for this". Wikidata is CC0, so there is no attribution obligation, no six-month retention window and no negotiation standing between today and a phone with a working app on it. Every row keeps its TMDB id, so the adapter enriches these rows in place when it arrives rather than building a second catalogue beside them.

Two absences are the trade rather than a to-do list. There are no posters, because a poster is not a free work and Wikidata has none to give — which means the client has to look right without artwork, and it is better to learn that now than after screens assume it. And `popularity` is null, because PRD §19 defines that as the provider's score and sitelink count is not the same measure; writing it there would have been a small lie that something downstream would eventually rank by.

The catalogue ships as a **generated migration**, which is the part worth defending. Data in a migration is unusual, but `supabase db push` is already how every environment gets its schema, the test harness already replays every migration, and `app_config` is seeded the same way — so a seed arriving by that route needs no second mechanism and no step anyone can forget before a build. The generator is committed alongside the dataset it reads, the SQL is never hand-edited, and a refresh is a new generated file whose upserts correct the same rows. That they correct rather than duplicate is asserted, because a mistaken conflict target would double the catalogue quietly.

Two things had to change to accommodate it. `media_items` gained a `provenance` column, because PRD §19's six-month window applies to TMDB rows and not to these, and `fetched_at` alone cannot tell them apart; its default is `'tmdb'` deliberately, since expiring a row that need not expire costs a refetch while the opposite mistake is a licence breach. And the test harness now negates the TMDB ids fixtures ask for: a test requesting 1018 had started failing on a unique index because a real film has that id, and a test should not break over what someone else's catalogue happens to contain.

The harness also began caching. Replaying two thousand seed rows for every test file took the database suite from forty seconds to nearly two minutes, which is how a suite stops being run; it now applies the migrations once per process and reloads a snapshot, back down to about a minute. The snapshot is taken from the real migrations in the real order rather than from a schema dump maintained beside them, so it cannot drift into disagreeing with production.
