...
| Alpha catalogue | **Decided 2026-08-13** | Engineering | The catalogue for private testing is **seeded from Wikidata**, not fetched from TMDB: ~380 films, ~190 series, ~1,400 seasons, CC0, shipped as a generated migration. Chosen because nothing could enter `media_items` at all while the adapter was unwritten, and because a CC0 catalogue carries no attribution obligation, no retention window and no licence question to settle first. Every row keeps its `tmdb_id`, so the adapter later enriches these rows in place rather than starting again | When the adapter ships, or the catalogue's coverage stops being enough to test with |
