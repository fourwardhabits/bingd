...
**Version:** v0.6
**Last updated:** 2026-09-26 (current-state amendments against `9d9683a`; §15, the stopping point, 2026-09-23)
...
| ~~INF-5~~ **SETTLED 2026-09-11** | Activation is defined as "ranked 10 titles" with no time bound for attribution, and "ranked 10 titles within 24 hours" for the activation-rate metric. **Split by the founder on 2026-09-11:** the *metric* keeps both numbers; **invite** activation is now "ranked 5 titles", the completed *Your First Five* | v0.5 used two near-definitions; neither was chosen by the founder. The split is a founder decision, not an inference | One canonical definition is needed for invite and share attribution to be reportable — and the two questions turned out to be different questions once onboarding existed |
...
| Bucket display copy | **Decided 2026-08-23 — the rewording the row above anticipated** | Founder | The three buckets now read **I liked it / It was fine / I didn't like it**. **Display copy only.** The stored `taste_bucket` values stay `loved` / `fine` / `not_for_me`, as do the RPC arguments, band ordering, score thresholds, recommendation and Taste Match semantics, and the analytics `bucket` property. One source of truth, `BUCKET_LABEL` in `features/collection/score.ts`; nothing branches on the strings | If the first cohort reads them wrong |
...
| TV progress | **Decided 2026-08-24, superseding the 2026-08-13 rule** | Founder | **Ranking a TV season is the completion claim.** The "How was it? — I liked it / It was fine / I didn't like it" that opens the flow already says the reader watched the season, so there is no Completed prerequisite, no Watching/Completed control, and no episode tracking. The season list stays *Ranked / Not ranked yet*. `progress` is left dormant and is **not** written when a season is ranked: its only consumers are a `clear_watch_date` check a ranked title already satisfies through its bucket, and a `season_completed` feed event with no writer — and a second partial record of "watched" is precisely what `deferred-roadmap.md` §19 needs to avoid. The superseded rule ("a season becomes rankable only when marked completed") was unenforced at every layer and depended on a state no user could enter, so this resolves the contradiction on the documents' side rather than by building a gate. Fenced by `ranking-contract.test.mjs` §TV-1 | Closed. Revisit only with repeat-watch history (`deferred-roadmap.md` §19) |
...
| **Logged collection visibility** | **Decided 2026-08-13**, trailing clause superseded | **Founder** | **The Logged collection and its buckets inherit profile visibility — public on a public profile, approved followers only on a private one.** The collection is part of the profile and follows the same rules. ~~Notes and watch dates stay private on both; the watchlist stays private at every visibility level~~ — **watch dates** stay private on both and that stands; the **watchlist** now inherits profile visibility (2026-08-20) and a **note** can be published by its author as a review (2026-08-23) | If alpha testers treat a bucket as more private than a ranking |
| Always-private fields | Required | Best practice | **Watch dates**, import history, email, account identifiers, capability state. ~~notes, watchlist~~ — both moved; see the two rows below | Never |
| Watchlist visibility | **Decided 2026-08-20** | Founder | The watchlist inherits profile visibility, on the same terms as rankings and the Logged collection. One line of RLS: `watchlist_own` became `watchlist_read using (can_i_view(user_id))`, migration `20260820000200` | — |
| Note and Review visibility | **Decided 2026-08-23; server failsafe added 2026-08-24; client default amended 2026-09-06 (#111), restored 2026-09-25 (#210)** | Founder | **A note is private until its author publishes it as a review.** A published note is a Bingd Review, attributed, beside the author's score, and is still gated by account visibility and by blocks — publishing widens a note as far as the account allows and never further. ~~**New notes default to private** in the client;~~ **Amended 2026-09-06 (#111), current at `9d9683a`:** a reader's first-ever new note opens with *Share as a review* on, and each later new note opens on the reader's last explicit choice for a new note. That choice is stored per account on the device (`note-visibility-pref`) and is not synced. The same rules hold on every note surface since #210; the control names the publishing act (*Share as a review*) rather than its absence, and the Reviews tab's "Write a review" opens ready to publish. A stored note always opens on the visibility it was saved with. **The server agrees since `20260825000200` (NR-1):** a brand-new note arriving with visibility absent or null is written **private**, in both `log_watched` and `save_note`, where both used to read a null as public. That is a safety invariant and not a change of direction — the app still nudges *Write a review* first — and it changes no stored row, because the branch fires only for a note that does not yet exist. An existing Review stays public through an edit that names no visibility, which is the half that is easy to break while fixing the other. PRD §22 | If reviews dry up enough to matter, measured rather than assumed |
| Account discovery | **Decided 2026-08-19** | Founder | **A private account is findable by handle or display name and can be sent a follow request**, disclosing five identity columns and nothing else. `search_users` and `profile_identity` gate on `can_discover_profile`, which reads status and blocks and ignores `visibility`. Private gates content, not existence | — |
...
| **Date of birth: retention and use** | **Decided 2026-08-24** (DOB-1) | Founder | **The exact date of birth is kept, privately, in the model it already has.** `profile_private` stays; the three minimisation options in `open-questions.md` §DOB-1 — discard after the comparison, keep the year only, ask for an age confirmation — are all declined, because year precision loses the boundary case that makes the 13+ gate honest and discarding forecloses the other two purposes. **Retained for three purposes:** 13+ account eligibility; future personalisation of recommendations by age; future aggregate or de-identified analysis of taste by age cohort. **The fence:** no widening of read access — nobody may read another account's date of birth and neither may its owner; raw DOB never leaves the database, not to PostHog, Sentry, feed events or ordinary recommendation payloads; future consumers take **derived** features (current age, age band) rather than the date, because a recommender needs "how old" and never "which day"; no demographic analytics tables; and **no new demographics at all** — not gender, race, ethnicity, religion, politics, sexual orientation, income or precise location. If optional recommendation inputs are ever added, the low-risk candidates are country/region and viewing languages, recorded as a direction and not a plan. **Under-13s may not hold Bingd accounts**, and the check stays exactly-calendar and server-side in `create_profile`. Store declarations are unaffected: Apple, Google Play and the privacy page already assert that Bingd stores a date of birth, and all three stay true. Signup copy widened to match, with the personalisation clause deliberately in the future tense because nothing reads the column but the age gate today | Revisit if a recommender is built, at which point the derived-feature rule is the thing to hold to |
...
| ~~Letterboxd import stage~~ | ~~**Decided for public alpha (changed from v0.5)**~~ | Founder | ~~Ships in v1, not early traction~~ — **superseded 2026-08-23**, see the row below | — |
| Letterboxd import stage | **Decided 2026-08-23 — deprioritized** | Founder | **Not a requirement for the friend beta, the initial App Store release, or the initial Play production release.** The specification in PRD §12 is unchanged and still canonical; only the stage moved. Reasoning and revisit trigger in `deferred-roadmap.md` §20 | Public traction, and real exports in hand to tune matching against |
...
| ~~Rating translation~~ | **Superseded 2026-09-21** — see *Letterboxd stars* in §15 | Founder | ~~Stars auto-map to buckets: 4.0+ → Loved it, 2.5–3.5 → It was fine, ≤2.0 → Not for me.~~ **A Letterboxd star is never a bingd bucket, position or score.** It is kept as provenance only, and the server refuses a bucket from any import (`20261018000100`) | — |
| Unrated watched films | Decided | Founder | Import as Logged with no bucket. Bucketed lazily by the user later. **Since 2026-09-21 this is true of every imported film, rated or not** | — |
| ~~Post-import anchor session~~ | **Superseded 2026-09-21** by the Unified Backlog (§15) | Founder | ~~A guided session over ~20 titles from the top of *Loved it*.~~ Imported titles arrive unranked and are ranked in the one Unified Backlog flow (Collection ▸ Unranked ▸ *Start ranking*). There is **no separate "Rank your imports" flow** | — |
| Unranked prompt | Decided for public alpha; **copy and dismissal settled 2026-08-23**; **superseded while `ranking.backlog_enabled` is on (2026-09-21)** — the Watched card reads *You have titles left to rank* with no count, Unranked carries *Start ranking* with the exact count, and the 50-ranked rule is dropped; see `refine-rankings-t5.md` §0 | Founder | A quiet, dismissible card on Collection. **Never a modal, never a progress bar toward 100%.** It is drawn only when *this side of the Movies/TV selector* actually holds unranked titles, and goes quiet at ~50 ranked titles. Copy now names its own reason — *"You have unranked titles" / "Rank them to complete your Collection and improve your recommendations." / "Rank now"* — replacing a line that advertised the app rather than describing the reader. Dismissal is an **X**, stored locally per account. **One re-eligibility rule: it returns only when fourteen days have passed *and* the reader has ranked something since.** Dismissing hides the card only; the Unranked tab stands while anything is unranked | — |
| Collection category memory | **Decided 2026-08-23** | Founder | Collection reopens on whichever of Movies / TV seasons the reader last chose, remembered locally per account. Movies stays the first-ever default. **Local preference only** — no backend table, no account preference, no migration. Scoped to Collection; For You keeps its own selector because it recommends *shows* where Collection lists *seasons* | If a shared preference across surfaces is ever wanted deliberately |
| Import list handling | **Decided (new in v0.6)**; **narrowed 2026-09-23** | Founder | **All lists import regardless of the list limit.** Nothing is ever deleted or hidden because of a limit. *Scope, as read at the stopping point:* "the list limit" is the in-app (tier) list limit, and imported lists are still exempt from it. The deferred Letterboxd Lists import (T6c) adds a separate **per-import ceiling of 100**, governed by the founder's 2026-09-23 decision in §15: above 100, the first 100 are imported deterministically and the skipped count is reported. This row and that decision should be confirmed together (see `open-questions.md`) | At T6c build |
...
| Rewatches | Provisional for import; **the product model is Decided 2026-08-23** | Founder | Import still takes the most recent watch date and ignores rewatch flags. Separately, the app's own repeat-watch semantics are now settled in principle: **"I watched this again" and "I want to change where this ranks" are different acts and neither implies the other.** A title keeps exactly one current ranking and one derived score; viewings become append-only watch events. Canonical design, migration and rollback in `deferred-roadmap.md` §19 — ~~designed, not built~~ **SHIPPED 2026-09-23 as Watch History** (#196; design `watch-history-and-ranking-calibration.md`, which resolved §19's open decisions). The Letterboxd diary's rows and Rewatch flag now become watch events (T1, R3) | — |
...
| Invitation acceptance | **Superseded 2026-09-08 — see the row below** | Founder | Explicit tap after the recipient has an account. Creates a ~~**one-way follow from recipient to inviter**. If the inviter is private, it creates a follow *request*. The inviter is then prompted to follow back — never automatically.~~ Recipient identity is not revealed to the inviter before acceptance. An active block in either direction voids the invitation | — |
| Invitation acceptance is bilateral | **Decided 2026-09-08 (supersedes the row above)** | Founder | A redeemed **personal** invite creates **both** follow edges, both `approved`, whatever either account's visibility says, and upgrades a pending edge in either direction. Minting a personal link and handing it to a named person is that approval, given in advance. The inviter is told at most once and has nothing to follow back. A future **referral** token keeps the one-way rule | If a leaked link ever proves to be a real abuse vector, per-recipient tokens |
...
| Custom lists in v1 | **Decided (resolves v0.5 contradiction)**; **superseded 2026-09-19** (`lists-prd.md` §P.1) | Founder | ~~Lists ship in v1 with the three-list limit enforced for everyone.~~ Native Lists v1 shipped (2026-09-23) with **no user-visible limit**: the hypothetical three-list limit is measured (`would_have_exceeded_3_lists`) and never shown; a 100 in-app ceiling exists as abuse protection. `unlimited_custom_lists` is defined but **not granted** | Grant only for a deliberate, time-boxed test |
...
| Alpha catalogue | **Decided 2026-08-13** | Engineering | The catalogue for private testing is **seeded from Wikidata**, not fetched from TMDB: ~380 films, ~190 series, ~1,400 seasons, CC0, shipped as a generated migration. Chosen because nothing could enter `media_items` at all while the adapter was unwritten, and because a CC0 catalogue carries no attribution obligation, no retention window and no licence question to settle first. Every film and series keeps its `tmdb_id` and every row keeps its Wikidata id, so the adapter later enriches these rows in place rather than starting again; seasons have no TMDB id to keep, because Wikidata publishes none, and match through their parent series and number | When the adapter ships, or the catalogue's coverage stops being enough to test with |
...
| Passwords | **Decided 2026-08-26, after a same-day amendment and its reversal** | Founder | **Ordinary users have no password in v1** — not created, set, changed or reset — and the email method is one six-digit code serving a new address and a returning one alike (`shouldCreateUser: true`, `verifyOtp({ type: 'email' })`), so nobody declares "sign up" or "sign in" before typing an address. An amendment earlier the same day made email-and-password the default on the reasoning that a password sends no mail; it is **reverted**. What is retained is Supabase's password *capability*, behind a deliberately quiet *More sign-in options → Sign in with password*, for **one** provisioned App Store / Play review account — a reviewer has no mailbox to receive a code in, so an OTP-only app cannot be reviewed. That screen cannot create an account and gives one generic refusal for every failure. Runbook: `../release/store-review-access.md`; contract: `../architecture/auth.md` §8 | If a real user population ever asks for a password, which would mean building a reset flow first |
...
## 15. The stopping point, 2026-09-23

What shipped in the stopping-point release (#196 Watch History + native Lists, #203 Unified
Backlog + Refine, #192 Android public distribution, #200 larger Letterboxd archives), and the
decisions it rests on. **Every row records a decision already made.** None is new scope.
Production: 168 migrations, head `20261019000100`, and the client in the OTA from `a17880d`.

| Area | Status | Authority | Direction | Source |
|---|---|---|---|---|
| Rewatch vs rerank | **Decided** | Founder | *Log another watch* appends a `watch_events` row (a rewatch). *Update your rating* is a **correction**: it writes no watch, no feed event and no streak week, and it keeps `rankings.created_at`. Neither implies the other | `20261001000100`, `20261005000100`; `correction-is-not-a-ranking.test.mjs` |
| Per-watch semantics | **Decided 2026-09-21** | Founder | Each watch has its own date and private watched-with. A private per-watch note column exists, but since #210 (2026-09-25) every note composer writes the one title-level note instead. Each watch's feed post keeps the score held **at** that watch. A pure correction moves the **most recently logged** watch's post only; earlier watches freeze. The review stays title-level. There are no per-watch ratings | `20261014000100`–`20261016000100`; `behavioural-contract.md` §1.2–1.3 |
| Exact ordinal is canonical | **Decided** | Founder | `rankings.position` is the single current truth. The score is derived from it. History is never averaged or replayed into it; the placement ledger records moves and never decides them | `architecture/ranking.md`; `watch-history-and-ranking-calibration.md` §E |
| Approximate placement | **Decided** | Founder | A title is placed approximately **only** when repeated **Can't decide** or exhausted eligible opponents leave a range. It lands at the uncertainty-safe midpoint, marked `adjustable` in the ledger (Refine's `fragile` boost). No comparison, tie or winner is invented. The reveal does not apologise for it (2026-08-30) | `20260901000100`, `_rank_finalize`; `RankingSheet.tsx` |
| No age-based staleness | **Decided 2026-09-21** | Founder | How long ago a title was placed is not evidence it is misplaced. Refine has no age term and no `placed_long_ago`. Its signals are **evidence gaps**, **newer contradicting answers**, and titles an **explicit rerank** carried past | `20261019000100`; `refine-rankings-t5.md` §2 |
| Unified Backlog | **Decided 2026-09-21** | Founder | One ranking session for everything seen and unranked (imports included). It supersedes the standalone "Rank your imports" / T6 queue. Backlog placements never post one by one, and a soft checkpoint comes every 10. **Amended 2026-09-24/25 (#208, #209):** Done shows a summary of what the sitting completed, and the sitting posts **one** grouped `ranking_batch` activity when it ends, naming the last title completed. Refine never creates one | `refine-rankings-t5.md` §0 |
| Comparison controls | **Decided 2026-09-22** | Founder | **Can't decide** declines this comparison and keeps placing the title. **Skip title (left)** on a comparison (**Skip title** on *How was it?*) drops the title for the sitting. The spacious comparison view is shared by every ranking surface | `RankingSheet.tsx` |
| Refine completion | **Decided 2026-09-22** | Founder | No per-title result page. A round (5 titles or 12 answers) ends on a summary (`#X → #Y`, current score). **Keep going** appears only while a fresh read finds strong titles, and there are at most 3 rounds. Done quiets the card until 3 new placements and a strong batch. Nothing returns on a timer | `refine-rankings-t5.md` §4, §6 |
| Refine rollout | **Decided** | Founder | Built and in production. `ranking.refine_enabled` stays **false** until the backlog smoke passes, and flipping it is the founder's call | `release/stopping-point-cutover.md` phases 5–8 |
| Letterboxd stars | **Decided 2026-09-21 (canonical)** | Founder | A star is **never** a bingd bucket, position or score. It is provenance only | `20261018000100` |
| Native Lists v1 | **Shipped** | Founder | Mixed media (movies, seasons and whole series), optional numbering, custom order (drag), the three privacy words, Share, Add to list, a public page. Managed in Collection's Lists mode; public lists shown on the Profile | `lists-prd.md` §R |
| Letterboxd Lists import (T6c) | **Deferred post-freeze** | Founder | A deliberate acquisition/migration feature, **not** unfinished core work. The 2026-09-22 NO-GO was accepted | `letterboxd-lists-import.md` §D |
| T6c: "edited since import" | **Decided 2026-09-23** | Founder | Any **user-initiated** rename, visibility change, reorder, or add or remove of a title. System and moderation changes (for example `hide_list`) do not count | `letterboxd-lists-import.md` §D.3 |
| T6c: more than 100 lists | **Decided 2026-09-23** | Founder | Import the **first 100 deterministically** and report the skipped count. The whole archive is never refused for it. The order that defines "first" is still to be specified | `letterboxd-lists-import.md` §D.3 |
| Deferred at the stopping point | **Deferred** | Founder | Letterboxd Lists import; predicted score; unmatched-import repair; *add what I've already seen* bulk entry; stats / Wrapped; real release notifications; Watch Next; post-ranking share cards; further Awards. **Still deferred at `9d9683a`, and none of it is planned next** | `deferred-roadmap.md` (stopping-point summary) |
| Still out of scope | **Unchanged** | Founder | Episode-level tracking; whole-series ranking (seasons are the TV unit) | §2 *Media scope*; §14 |
