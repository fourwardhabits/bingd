...
**Version:** v0.6
**Last updated:** 2026-09-23 (§15, the stopping point)
...
| Note and Review visibility | **Decided 2026-08-23; server failsafe added 2026-08-24** | Founder | **A note is private until its author publishes it as a review.** A published note is a Bingd Review, attributed, beside the author's score, and is still gated by account visibility and by blocks — publishing widens a note as far as the account allows and never further. **New notes default to private** in the client; the control names the publishing act (*Share as a review*) rather than its absence, and the Reviews tab's "Write a review" opens ready to publish. A stored note always opens on the visibility it was saved with. **The server agrees since `20260825000200` (NR-1):** a brand-new note arriving with visibility absent or null is written **private**, in both `log_watched` and `save_note`, where both used to read a null as public. That is a safety invariant and not a change of direction — the app still nudges *Write a review* first — and it changes no stored row, because the branch fires only for a note that does not yet exist. An existing Review stays public through an edit that names no visibility, which is the half that is easy to break while fixing the other. PRD §22 | If reviews dry up enough to matter, measured rather than assumed |
...
| Per-watch semantics | **Decided 2026-09-21** | Founder | Each watch has its own date, private note and watched-with. Each watch's feed post keeps the score held **at** that watch. A pure correction moves the **most recently logged** watch's post only; earlier watches freeze. The review stays title-level. There are no per-watch ratings | `20261014000100`–`20261016000100`; `behavioural-contract.md` §1.2–1.3 |
...
| Unified Backlog | **Decided 2026-09-21** | Founder | One ranking session for everything seen and unranked (imports included). It supersedes the standalone "Rank your imports" / T6 queue. Backlog placements never post to the Feed, and a soft checkpoint comes every 10 | `refine-rankings-t5.md` §0 |
...
| Native Lists v1 | **Shipped** | Founder | Mixed media, optional numbering, custom order (drag), the three privacy words, Share, Add to list, a public page. Managed in Collection's Lists mode; public lists shown on the Profile | `lists-prd.md` §R |
...
| Deferred at the stopping point | **Deferred** | Founder | Letterboxd Lists import; predicted score; unmatched-import repair; *add what I've already seen* bulk entry; stats / Wrapped; real release notifications; Watch Next; post-ranking share cards | `deferred-roadmap.md` (stopping-point summary) |
