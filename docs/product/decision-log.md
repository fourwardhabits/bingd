...
| TV progress | Decided — **and contradicted by the build as of 2026-08-23** | Founder | A season may be marked *Watching*, but becomes rankable only when marked completed. **This is not what ships.** `set_season_progress` exists and is granted but has **zero client call sites**, so `user_media.progress` is unreachable by any user; `set_bucket` and `rank_start` gate only on *series vs season* and never read `progress`; and the series page shows *Ranked / Not ranked yet* where PRD §9 specifies *Watching / Completed*. The rule is therefore unenforced at every layer and the state it depends on cannot be entered. Full statement of the contradiction in `open-questions.md` §TV-1 | **Open.** Needs a founder call on whether the gate is real before either the UI or the guard is built |
...
| Note and Review visibility | **Decided 2026-08-23** | Founder | **A note is private until its author publishes it as a review.** A published note is a Bingd Review, attributed, beside the author's score, and is still gated by account visibility and by blocks — publishing widens a note as far as the account allows and never further. **New notes default to private** in the client; the control names the publishing act (*Share as a review*) rather than its absence, and the Reviews tab's "Write a review" opens ready to publish. A stored note always opens on the visibility it was saved with. PRD §22 | If reviews dry up enough to matter, measured rather than assumed |
