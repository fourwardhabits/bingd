...
**Current state (2026-09-26, `9d9683a`, frozen):** Part A's client (the optional note on a
recommendation) is in the frozen source. Part B, **Watch next, remains parked and deferred**:
no `watch_next` table, RPC or client exists on main, and it is not planned next.
