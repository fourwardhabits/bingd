...
| Event | Fires exactly when | Owner | Properties |
|---|---|---|---|
| `for_you_slate_shown` | a genuinely new wall was put in front of the reader — once per distinct slate, from the same guard `noteImpressions` uses, never per render or per page already recorded | the reader | `medium`, `size`, `repeat_count`, `liked_titles`, `anchors`, `pool_size` |
...
`liked_titles`, `anchors` and `pool_size` (2026-09-13) say what the wall had to rotate
