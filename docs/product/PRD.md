...
**A correction is not another watch — founder decision 2026-09-07 (`20260911000100`, written, not yet applied).** The Ranked menu's *Adjust placement* and *Change your rating* replace a position and post no activity ([§10](#10-ranking-system), 2026-08-26). They are also not watch signals for the watchlist: a title the reader ranked and then deliberately put back on the watchlist stays there through either correction, for the exact object and for its series. Until this migration both removed it, because a correction is performed as a delete-and-insert of the ranking row and the departure triggers heard the insert as a first ranking — the same older-signal-overrules-newer-intention mistake the 2026-08-15 invariant had already corrected for a note edit and a `watching` progress. *I watched it again* is a watch and takes the entry as it always did. The same migration keeps `rankings.created_at` — the instant the title entered the ranking, which the sort contract of 2026-08-30 defines as the instant its activity carries — unchanged through a correction, so *Recently ranked* and the weekly streak see a non-event as a non-event. The mechanism is a transaction-local marker set by the one function that knows it is correcting, never a client flag; `supabase/tests/rerank-keeps-watchlist.test.mjs` pins every case above.
...
### As built — 2026-08-28: the award loop — a crossing is announced exactly once (`20260828000100`)
...
| You earned an award *(row added 2026-08-28; no writer existed before `20260828000100`)* | Yes | Yes |
...
| `awards` | `award_earned` | **on** *(off until 2026-08-28)* |

`reactions` defaults off because it is the only event whose median notification carries nothing beyond "somebody saw this". `awards` defaulted off because **nothing wrote one**; `20260828000100` gave `award_earned` its writer and flipped the default to **on** through the `20260820000100` mechanism — no data migration, and an explicit choice already made in settings is preserved.
...
- ~~**`award_earned` has no writer**, and this is a disposition rather than an omission. Award tiers are computed entirely on the device from raw table reads; **no durable state records which tier an account has reached**, so a *crossing* cannot be distinguished from a *state*, and exactly-once delivery is impossible without an unlock ledger. An award notification that fires twice is worse than one that never fires.~~ **Superseded 2026-08-28**: `award_unlocks` is that ledger, `20260828000100` is the writer, and the exactly-once demand the deferral insisted on is met by insert-wins on its primary key. §14's As-built block dated 2026-08-28, and the closed [`deferred-roadmap.md`](./deferred-roadmap.md) §5.
...
>
> **Acceptance semantics above are implemented in full.** Redemption writes the attribution, creates the follow of clause 2 and notifies the inviter (clause 4). ~~files a **request** instead when the inviter is private (clause 3) … who is never auto-followed.~~ **Clauses 2, 3 and 4 were superseded on 2026-09-08**, and what is implemented is the superseding rule: a **personal** invitation creates both edges, both `approved`, whatever either account's visibility says, and the inviter is told at most once. A **referral** token still does what clauses 2 and 3 describe. See the As-built block at the end of this section. Clauses 1, 5, 6 and 7 were already met: the tap is explicit, the recipient is unnamed until it commits, a block voids the invitation, and the attribution is written independently of the follow.
>
...
> a **draft**: as of 2026-09-04 it names its operator (Suraj Kandukuri, using the
> developer name FourwardStudios, with no company involved), but it still states no
...
- [ ] **HG-4** Privacy policy, terms of use, support contact, 13+ statement, age ratings, and data-request path published. **Terms of Use drafted and routed 2026-08-25** at `/terms`, generated by the same `web/build.mjs` as the privacy, support and deletion pages, linked from Settings and acknowledged at account creation. **It is a draft and this item stays open**: the operator was settled 2026-09-04 (Suraj Kandukuri, using the developer name FourwardStudios; no company), but it states no governing law, venue or arbitration clause, and no lawyer has read it. Age ratings and the data-request path are untouched by that tranche.
