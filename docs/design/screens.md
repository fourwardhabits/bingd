...
- **As built — 2026-09-13 (founder, preview QA Round 3): Already use Letterboxd?** An optional step of its own between *Your First Five* and People (`app/onboarding/letterboxd.tsx`), replacing a passive card on the payoff that went unnoticed. Heading *Already use Letterboxd?*, one line on what comes over (watched movies, ratings, diary dates, watchlist), the importer's privacy line, then **Import from Letterboxd** (primary, straight to the file picker), **Not now** (secondary) and *Need help getting the file?* (the instructions sheet). The real importer runs in place — preview, upload, refusals — with *Not now* on every state short of the upload. Once the server has the import, the step says it keeps running in the background and that we will say when it is done, and offers **Continue**; it never waits for the result. The payoff keeps its single action. See [`letterboxd-import.md`](../product/letterboxd-import.md) §1.
...
**As built — 2026-09-12, what a placement leaves behind.** Since the pick, rank loop (PR #133) onboarding draws no per-title reveal: a placement returns straight to the picker, and *Your First Five* is the first screen that shows scores together. What replaced the reveal was nothing, and on a device that read as a sheet flashing up and away. Now the picker carries one line under the progress, a small poster and *Inception landed at 8.7*, which stays until the next title is chosen: no Done, no timer, and not a sheet. An empty band, which places outright, never presents the comparison sheet at all (while the answer is on its way the same slot reads *Ranking Inception…*); a sheet that asked a question slides out still showing its last pair rather than an empty strip. The fifth placement goes to *Your First Five* exactly as before. The sheet serialisation that keeps two modals from overlapping (`Sheet`'s `onShown` / `onDismissed`) is unchanged.
...
> **Renamed 2026-09-22 (founder QA of the Unified Backlog), superseding "final" above: the control is `Can't decide`** (accessibility label "Can't decide between these"). Beside the ranking session's own **Skip title (left)**, which drops the title for the sitting, "Too tough" read as a second skip. The mechanism is unchanged: the same `rank_skip`, three-skip midpoint and `adjustable` mark. The comparison view is the shared, spacious one that sizes its cards from the window. See `../product/PRD.md` §10 and `../architecture/ranking.md` §5, §13.
...
Pressing and holding either poster opens a compact sheet inside the comparison: the full name, the year, certification, length, genres, and who directed or created it, then the overview clamped to three lines with `… more`. Nothing to act on — no score, no watchlist control, no reviews — because an action here would be a second decision competing with the one the reader is in the middle of. Dismissing returns to the same pair, because the reminder renders *inside* the comparison and nothing about the pair was ever unmounted.

React Native suppresses `onPress` after a long press, so holding a poster to read about it cannot also register as choosing it — the correctness property this gesture needed. A long press is invisible and unreachable to a screen reader, so each card also carries a small **Details** control with its own label, per design-system.md §8's rule that a hidden gesture may be the fast path and never the only one. Its hint names the sheet that will open, and that is a different sheet for each kind — see below.

**The memory section, and why the two kinds diverge (2026-09-11).** Everything above is a fact about the *title*, which is the right set for a film and close to useless for a season: a season comparison is routinely Season 1 against Season 4 of one show, and against that question the poster is the same artwork with a different number, the genres and the certification are inherited from the parent, and the cast are the same people in every season. So the sheet ends with one section that is different for each.

- **A film** gets its **backdrop** — full width, 16:9, not tappable — followed by three cast names. A synopsis tells you the plot of a film you watched in 2019, and the plot is the thing you forgot. It is **one** image and never a strip: TMDB has no movie equivalent of an episode still, and the movie images endpoint returns key art rather than scenes, often the same composition in several languages and several with the title burned into the picture, so five of them would be five crops of one frame. That endpoint is not called and no provider request is added.
- **A season** gets its **episodes**, six of them, drawn with the same `EpisodeRow` the season page uses: number, name, air date, still, and a three-line synopsis. Past six, **Show all N episodes** expands the list *in place*. The cast line is dropped once the episodes arrive and kept as the fallback when the list is empty or the fetch failed — a season that loses its episodes must not end up with less than the sheet showed before.

Nothing in this section opens a second sheet, navigates to the title page, or offers a fullscreen image. The sheet is already a modal presented from inside the ranking's own, and a third presentation is the shape of the freeze design-system.md §8 describes.

**A season also says which show it is.** `media_items.title` for a season is the bare words "Season 1", and the heading printed them: a reader who pressed Details on a season to work out which show it was got a poster and a season number. The parent has been selected since the sheet shipped and was simply never passed to `compactName`. Fixed 2026-09-11.

It adds one data path and no new provider surface. The row comes from `media_items`, which the comparison card already reads, and the backdrop is a column on it that was not being selected; the credits come from the `credits` facet of `media_cache`, which the title page has read since the integration landed. The episodes read the same episode cache the season page seeds on mount, so a reader who looked at that page in the last hour pays nothing, and when the cache is cold it is one `season-episodes` call against the reader's own hourly ceiling. A film never asks for episodes at all. Nothing is fetched until somebody actually presses Details.
...
### As built — 2026-09-14: the Leaderboard is people competing, and nothing else

A `People | Top Titles` tab row sat here from 2026-09-13 until physical QA; the founder removed
Top Titles the next day. **The Leaderboard is a competition between people**: This month / All
time with the per-account remembered timeframe, Titles · Movies · TV · Reviews, Match, privacy.
Top Rated titles stay where they already were, on For You (Top Rated Movies · Top Rated TV), and
there is no second surface for them: a community score beside a people competition emphasised
the number in exactly the place ranking behaviour is being encouraged. The backend it read,
`top_rated_titles` with the shared `community_support_floor` (`20260916000200`), is unchanged
and still serves For You.

**One layout skeleton for every metric.** Switching Titles · Movies · TV · Reviews must not move
the board. The loading skeleton and the empty state used to sit in a container with extra top
padding and a second horizontal inset, and the empty state drew its full-size title — so a metric
with rows (usually Titles) and a metric without (often Reviews) started at different heights and
only one of them appeared to have a heading, and every switch went rows → shifted skeleton → rows.
Rows, skeleton and empty state now share one content container at the same top, the empty state is
the compact one, and a metric switch keeps the previous board on screen until the next arrives
rather than collapsing to a skeleton.

The community number stays out of the ranking flow: `src/features/ranking/no-community-anchor.test.ts`
fails if any ranking source reaches a community or following score.
