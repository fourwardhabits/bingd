...
React Native suppresses `onPress` after a long press, so holding a poster to read about it cannot also register as choosing it — the correctness property this gesture needed. A long press is invisible and unreachable to a screen reader, so each card also carries a small visible control with its own label, per design-system.md §8's rule that a hidden gesture may be the fast path and never the only one. *(The control's copy was **"What is this?"** when this was written and is **"Details"** since the final pre-RC pass — founder feedback from physical testing: a question under both posters reads as the screen being unsure rather than as an offer, and at 375pt the three words wrap asymmetrically under the two titles. One word cannot wrap. The record is the comment above the control in `RankingSheet.tsx`.)*
