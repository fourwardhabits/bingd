...
**The profile's social row (2026-09-15, moved 2026-09-16).** Full width under the identity block, the first glyph aligned with the bio at the gutter, in the order TikTok, Instagram, X, YouTube, Website. Five 32pt cells `space[3]` apart with 6pt slop on every side, so each answers 44 × 44 and neighbouring slops meet without crossing. (While it sat in the name column it was 44 × 40 for lack of width; moving it removed that concession.) Pinned in `touch-targets.test.tsx`. The glyphs are `icon.sm` in `text.secondary` — **never the platform brand colours**, which would put five companies' palettes above somebody's bio.
