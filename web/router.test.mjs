...
describe('the release mode', () => {
   ...
   *
   * **The shot list moved on 2026-09-11** with the landing page. It used to name
   * `shot-collection.jpg` and `shot-ranking.jpg`, which are the router's two and stay
   * the router's two; the front page ships its own set, generated by `web/shots.mjs`.
   * The assertion is the rule rather than the filenames it was written against: the
   * page carries the mechanic, the payoff and the social layer, and every picture on it
   * is described to somebody who cannot see it.
   */
  it('gives the front page an install row and the product shots', () => {
    ...
    for (const name of ['shot-compare', 'shot-reveal', 'shot-movies', 'shot-feed']) {
      ...
      assert.ok(
        ...
        `${name}.webp is referenced and not shipped`,
      );
    }

    // Every picture, including the masthead mark, is either described or explicitly
    // decorative. A thin-but-present alt is the failure worth catching: it passes a
    // linter and tells a screen reader nothing.
    for (const alt of front.matchAll(/<img[^>]*alt="([^"]*)"/g)) {
      assert.ok(
        alt[1].length === 0 || alt[1].length > 20,
        `a product shot has thin alt text: "${alt[1]}"`,
      );
    }

    // Width and height on every one, so the page does not reflow under a thumb as the
    // bytes arrive and move the install button out from under it.
    for (const img of front.matchAll(/<img\b[^>]*>/g)) {
      assert.match(img[0], /\bwidth="\d+"/, `an image has no width: ${img[0].slice(0, 60)}`);
      assert.match(img[0], /\bheight="\d+"/, `an image has no height: ${img[0].slice(0, 60)}`);
    }
  });

  /**
   * The install row is painted three times from one decision.
   *
   * The hero holds the ids and the closing band and the sticky bar mirror them through
   * `data-install`, which `page.mjs` resolves alongside `getElementById`. The failure
   * this pins is the one that would ship silently: a mirror whose name is misspelt is a
   * button that never appears, and an empty closing band looks like a design choice.
   */
  it('mirrors the install row rather than repeating the decision', () => {
    const front = read('index.html');
    const mirrored = [...front.matchAll(/data-install="([a-z-]+)"/g)].map((m) => m[1]);

    assert.ok(mirrored.length > 0, 'the front page has no mirrored install row at all');

    for (const name of new Set(mirrored)) {
      assert.match(
        front,
        new RegExp(`id="${name}"`),
        `data-install="${name}" mirrors a name no element on the page owns`,
      );
    }

    // And the mirror is resolved by the module that paints, not by hope.
    assert.match(read('page.mjs'), /data-install=/, 'page.mjs does not read the mirrors');

    // The three places: hero, closing band, sticky bar.
    assert.equal(
      (front.match(/data-install="primary-install"/g) ?? []).length,
      2,
      'the primary install button is not mirrored into both the closer and the sticky bar',
    );
  });

  /**
   * SEO hygiene, which is four tags and two files and stops there.
   *
   * Not a growth initiative: a title, a description, a canonical, the social card, and a
   * robots/sitemap pair. The last of those fixes a real defect rather than adding a
   * nicety — Cloudflare Pages answers an unmatched path with index.html and a 200, so
   * /robots.txt has been serving HTML to crawlers for as long as the site has existed.
   */
  it('gives the front page the hygiene tags, and the site a robots and a sitemap', () => {
    const front = read('index.html');

    assert.match(front, /<link rel="canonical" href="https:\/\/bingd\.app\/" \/>/);
    assert.match(front, /<title>[^<]{20,}<\/title>/, 'the title is too thin to be a title');
    assert.match(front, /<meta name="description" content="[^"]{80,}"/);
    assert.match(front, /<meta property="og:image" content="[^"]+social-card\.png"/);
    assert.match(front, /<h1>/, 'the front page has no h1');
    assert.equal((front.match(/<h1[ >]/g) ?? []).length, 1, 'more than one h1');

    const robots = read('robots.txt');
    assert.match(robots, /Sitemap: https:\/\/bingd\.app\/sitemap\.xml/);

    /**
     * **Nothing may be disallowed here**, and this is the half of it worth a test.
     *
     * The four routes that name an account are kept out of the index by X-Robots-Tag and
     * a robots meta tag. A Disallow would defeat exactly that: a crawler told not to
     * fetch a URL never reads the noindex on it, so a /u/<handle> already known to
     * Google would stay indexed with nothing able to withdraw it.
     */
    assert.doesNotMatch(robots, /^\s*Disallow:\s*\S/m, 'a Disallow would block the noindex');

    const sitemap = read('sitemap.xml');
    for (const path of ['/', '/privacy', '/terms', '/support', '/account-deletion']) {
      assert.match(sitemap, new RegExp(`<loc>https://bingd\\.app${path}</loc>`));
    }
    // And never a route that names an account.
    for (const path of ['/u/', '/i/', '/title/', '/lists/']) {
      assert.doesNotMatch(sitemap, new RegExp(path.replace(/\//g, '\\/')), `the sitemap offers ${path} to Google`);
    }
  });
   ...
   * where somebody is being persuaded. bingd. has no testimonials, no press, no install
   * count and no star average to state, and a sentence inventing one would read as
   * completely ordinary next to everything else here. So the shapes are refused
   * outright rather than left to a reviewer noticing.
   */
  it('puts no social proof on the front page, because there is none to put', () => {
    const front = read('index.html');
    for (const claim of [
      /\b\d[\d,.]*\s*(?:k|m|thousand|million)?\+?\s*(?:users|members|people|installs|downloads|ratings|reviews)\b/i,
      /join (?:thousands|millions|\d)/i,
      /\b(?:loved|trusted|used) by\b/i,
      /\b\d(?:\.\d)?\s*(?:out of|\/)\s*5\b/i,
      /as (?:seen|featured) (?:in|on)\b/i,
      /\bapp of the (?:day|week|year)\b/i,
    ]) {
      assert.doesNotMatch(front, claim, 'the front page grew social proof nobody can evidence');
    }
  });
