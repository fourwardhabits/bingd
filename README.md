...
The fix for those six functions came with a claim attached, and the claim was wrong — then the correction to it was wrong too, which is the more useful story.

`20260813001800` also ran `alter default privileges in schema public revoke execute on functions from public, anon, authenticated`, on the reasoning that new functions would then arrive closed and nobody would have to remember. The next function written was executable by strangers, and `function-grants.test.mjs` failed on it within seconds. Inspecting `pg_default_acl`: the `anon` and `authenticated` parts had taken effect and the `PUBLIC` part had not, so every function created since still carried `=X` — and since every role belongs to `PUBLIC`, `anon` reached it regardless.

The conclusion drawn from that was "the setting silently didn't work, so trust tests over settings." Satisfying, and wrong. A review went to the documentation, where **the PostgreSQL manual gives that exact statement as its example of a command that does nothing**: per-schema default privileges can only *add* to the global setting, never subtract from it, and `PUBLIC`'s execute comes from the built-in *global* default. The `anon` and `authenticated` revokes worked because they were undoing Supabase's own per-schema grants, which is the one case the manual carves out.

So the mechanism works and the wrong variant was used. `20260813002100` issues the global form — no `IN SCHEMA` — and it behaves correctly: verified by execution, a function created afterwards carries no `PUBLIC` grant.

The allow-list test remains the real guard, but for a stated reason rather than a vague one: default privileges attach to **the role that set them**, so the statement covers objects created by that role and nothing else. An extension, or an object created by `supabase_admin`, falls outside it. A CI check that sweeps the whole schema has no such boundary. Which is a narrower and more useful lesson than the one first written here — and a reminder that a confident diagnosis of surprising behaviour is worth ten minutes in the manual before it goes in a README.
